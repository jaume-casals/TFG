...
Després	de l'avaluació sense etiquetes, s'ha pogut comprovar que el model ha decidit comprometre una lleugera part de puntuació \textit{ROUGE} per aconseguir una última millora de les coincidències exactes.
...
El model Flan-T5-Base es va ajustar mitjançant el mètode \textit{LoRA} (\textit{Low-rank Adaptation of LLMs}), que es va centrar a entrenar únicament les matrius ``q'', ``v'', ``k'' i ``o'' del mòdul d'atenció del model. Aquests paràmetres entrenables només representen l'1,41\% de tots els paràmetres del model, reduint en gran manera la càrrega computacional.
\begin{table}[H]
    \centering
    ...
    \bottomrule
    \end{tabular}
    \caption[Entrenament del model \textit{Flan-T5-Base} amb \textit{LoRA} ]{Entrenament del model \textit{Flan-T5-Base} amb \textit{LoRA} (amb etiquetes)}
\end{table}
El \textit{fine-tuning} amb el mètode \textit{LoRA} mostra un patró clar en els errors de \textit{Train} i \textit{Val}, cosa que indica una millora gradual del rendiment del model al llarg de les \textit{epochs} especificades. No obstant això, les puntuacions ROUGE mostren una tendència diferent, cosa que suggereix que la capacitat del model per captar els matisos del \textit{dataset} i generar resultats rellevants es pot veure qüestionada en aquest entrenament. És per aquest motiu que es va decidir cancel·lar l'entrenament i no continuar amb el procés.
