...
\textbf{FLOR-760M}
##### CONTINUAR AQUI!!!!! #####
A causa del fet que el model \textbf{Flan-T5-Base} no va demostrar bons resultats en eliminar les signatures dels correus, es va decidr canviar de model. El model \textbf{FLOR} és un model de llenguatge causal basat en l'arquitectura transformers per a català, castellà i anglès. És el resultat d'una tècnica d'adaptació lingüística realitzada al model \textbf{BLOOM}, que implica modificar el vocabulari del model i la capa d'\textit{embedding}, i entrenar el model contínuament amb fitxes de 26B en les llengües objectiu. S'ha fet l'entrenament amb el model de 760M per 5 epochs.
...
A causa d'aquests problemes, s'ha decidit canviar el mètode per detectar les signatures en els articles dels tiquets.
